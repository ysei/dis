...
/* -J */
	...
	"	-N		サイズがデフォルトなら付けない\n"
/* -O */
	"	-P num		ソ\フトウェアエミュレーション命令を有効にする(ビット指定, 初期値=3)\n"
	"		+1	未実装浮動小数点命令を有効にする\n"
	"		+2	未実装整数命令を有効にする\n"
/*	"	-Q		環境変数 dis_opt を参照しない\n"	*/
	...
	"	-U		ニーモニックを大文字で出力する\n"
	"	-V num		バックトラックの原因の表\示(0:しない [1]:プログラム領域 2:全ての領域)\n"
	"	-W num		同一データを .dcb で出力する最小バイト数. 0なら圧縮しない(初期値=64)\n"
	"	-X		16 進数を大文字で出力する\n"
	"	-Y		カレントディレクトリからも include ファイルを検索する\n"
	"	-Z[num]		16 進数をゼロサプレスする([0]:通常 1:省略可能\な'$'を省略)\n"
	...
	"	--sp		a7 レジスタを'sp'と表\記する(標準では --a7)\n"
	"	--old-syntax	アドレッシングを旧表\記で出力する(標準では --new-syntax)\n"
	"	--(in)real	浮動小数点を[実数表\記](内部表\現)で出力する\n"
